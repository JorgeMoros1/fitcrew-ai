...
async def _handle_consultation(sender: str, message: str) -> None:
    ...
    try:
        write_conversation_turn("synthesizer", message, response)
    except Exception:
        log.error("write_conversation_turn [synthesizer] failed:\n%s", traceback.format_exc())
...
async def handle_message(sender: str, text: str) -> None:
    ...
    if agent_name == "coach":
        msg = clean_text
        log.info("CONSULTATION: @coach | msg=%r", msg)
        await _handle_consultation(sender, msg)
        return
    elif agent_name == "all":
